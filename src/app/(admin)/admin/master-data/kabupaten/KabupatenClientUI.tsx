...
import { createKabupaten, deleteKabupaten, updateKabupaten } from '@/app/actions/wilayah';
import Toast from '@/components/ui/Toast';
import ConfirmModal from '@/components/ui/ConfirmModal';

export default function KabupatenClientUI({ initialData }: { initialData: any[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    let res;
    if (editingId) {
      res = await updateKabupaten(editingId, data);
    } else {
      res = await createKabupaten(data);
    }
    ...
    if (res.success) {
      ...
      setToastMessage(editingId ? 'Berhasil mengubah data Kabupaten!' : 'Berhasil menambahkan data Kabupaten!');
    } else {
      setToastType('error');
  ...
  async function handleConfirmDelete() {
    if (!confirmId) return;
    setIsDeleting(true);
    const res = await deleteKabupaten(confirmId);
    setIsDeleting(false);
    setConfirmOpen(false);
    
    if (res.success) {
      setToastType('success');
      setToastMessage('Data Kabupaten berhasil dihapus!');
    } else {
      setToastType('error');
  ...
  const filteredData = useMemo(() => {
    return initialData.filter(item => {
      return item.namaKabupaten.toLowerCase().includes(searchQuery.toLowerCase());
    });
  ...
  return (
    <>
      ...
      <ConfirmModal 
        ...
        message="Apakah Anda yakin ingin menghapus data Kabupaten ini? Data yang dihapus tidak dapat dikembalikan."
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-8 relative shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              >
                {isSubmitting ? 'Menyimpan Data...' : editingId ? 'Simpan Perubahan' : 'Simpan Kabupaten'}
              </button>
            </form>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/80 text-slate-500 text-xs font-bold uppercase tracking-wider border-b border-slate-200">
                ...
                <th className="p-5">Nama Kabupaten</th>
              ...
              {filteredData.length === 0 && (
                <tr>
                  <td colSpan={3} className="p-12 text-center text-slate-500">
                    ...
                    <p className="font-medium text-lg">Data kabupaten tidak ditemukan</p>
                  </td>
                </tr>
              )}
            </tbody>
