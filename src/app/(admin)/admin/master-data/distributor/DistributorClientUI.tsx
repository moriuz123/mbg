...
export default function DistributorClientUI({ initialData }: { initialData: any[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await addMasterData('distributor', name);
      setIsOpen(false);
      router.refresh();
    } catch (error) {
  ...
  async function handleDelete(id: string) {
    if (confirm('Yakin ingin menghapus distributor ini?')) {
      try {
        await deleteMasterData('distributor', id);
        router.refresh();
      } catch (error) {
  ...
  return (
    <>
      ...
      >
        <Plus size={20} /> Tambah Distributor
      </button>
      ...
      {isOpen && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '1rem', width: '100%', maxWidth: '500px', padding: '2rem', position: 'relative' }}>
            ...
            <h2 style={{ marginTop: 0, marginBottom: '1.5rem', fontSize: '1.25rem', fontWeight: 700 }}>Tambah Distributor</h2>
            
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem', fontWeight: 600 }}>Nama Distributor</label>
                <input required name="name" type="text" style={{ width: '100%', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid #cbd5e1' }} placeholder="Contoh: PT. Logistik Nasional" />
              </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-sm border-b border-slate-200">
                <th className="p-4 font-semibold">Nama Distributor</th>
